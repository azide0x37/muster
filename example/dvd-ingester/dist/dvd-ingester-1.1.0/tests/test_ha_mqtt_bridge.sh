...
printf '{"state":"ready_for_cold_publish","run_id":"title-3"}\n' > "$ROOT/run/dvd-ingester/handoff.json"
printf '{"state":"healthy","reason":"drain_complete","published":1,"failed":0}\n' > "$ROOT/run/dvd-ingester/publish.json"
mkdir -p "$ROOT/var/lib/dvd-ingester/work/title-2.work.123" "$ROOT/var/cache/dvd-ingester/hot/title-3" "$ROOT/mnt/dvd-ingester/title-1" "$ROOT/mnt/dvd-ingester/.incoming-title-3-123"
...
printf '%s\n' old > "$ROOT/mnt/dvd-ingester/.incoming-title-3-123/payload.txt"
...
grep -q '"conveyance_status":"waiting_for_extraction"' "$ROOT/run/dvd-ingester/ha-mqtt-state.json"
