...
- Register the implementation with the shared Muster inspector and bootstrap
  the independently versioned `muster` command on the first installation.
- Add a schema 2 component graph, deterministic release lock, and the complete
  13-node T2R4 plus T2R6 closure at the recorded MPL commit.
- Separate runtime doctor inspection from the synthetic conveyor self-test and
  emit structured `muster.observation/v1` evidence.
- Restore the prior registry entry together with `/opt/dvd-ingester/current`
  when graph validation or the post-update doctor fails.
- Harden install and update as one serialized release transaction with private
  ...
  reuse, and atomic rollback of current, registration, units, and the udev rule.
